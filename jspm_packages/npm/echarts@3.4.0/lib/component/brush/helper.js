...
helper.resetInputRanges = function(brushModel, ecModel) {
  each(brushModel.brushRanges, function(brushRange) {
    ...
    if (__DEV__) {
      zrUtil.assert(!coordInfo || coordInfo === true || brushRange.coordRange, 'coordRange must be specified when coord index specified.');
      zrUtil.assert(!coordInfo || coordInfo !== true || brushRange.range, 'range must be specified.');
    }
    brushRange.range = brushRange.range || [];
    if (coordInfo && coordInfo !== true) {
      brushRange.range = coordConvert[brushRange.brushType](0, coordInfo, brushRange.coordRange);
...
helper.resetOutputRanges = function(brushRanges, brushModel, ecModel) {
  each(brushRanges, function(brushRange) {
    ...
    if (panelId) {
      ...
      brushRange.coordRange = coordConvert[brushRange.brushType](1, findCoordInfo(brushRange, brushModel.coordInfoList), brushRange.range);
    }
  });
};
